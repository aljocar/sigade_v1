...
const item_menu_asignar_seccion = document.getElementById('item_menu_asignar_seccion');
...
item_menu_grados_crear.addEventListener('click', (event) => {
  ...
  item_menu_asignar_seccion.addEventListener('click', (event) => {
    event.preventDefault(); // Evita que el enlace cargue una nueva página
  
    fetch('./assets/asignar_seccion.html')
      .then(response => response.text())
      .then(html => {
          container_contenido.innerHTML = html;
      });
